            ...
            <tbody>
                ...
                <tr class="">
                    ...
                    <td scope="row" width="15%" class="text-center align-middle">
                        <a role="button" class="btn btn-primary" href="{{route('admin.pokedex.show', $pokemon->id)}}">
                            <i class="fa-solid fa-eye"></i>
                        </a>
                        <a role="button" class="btn btn-secondary" href="{{route('admin.pokedex.edit', $pokemon->id)}}">
                            <i class="text-light fa-solid fa-pencil"></i>
                        </a>
                        <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modal-{{$pokemon->id}}">
                            <i class="fa-regular fa-trash-can text-light"></i>
                        </button>
                        <div class="modal fade" id="modal-{{$pokemon->id}}" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-labelledby="modalTitle-{{$pokemon->id}}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-sm" role="document">
                                <div class="modal-content">
                                    <div class="modal-header d-flex flex-column">
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="close"></button>
                                        <h5 class="text-muted fs-6 text-uppercase">You are going to delete</h5>
                                        <h5 class="modal-title mb-2 text-uppercase fw-bold" id="modalTitle-{{$pokemon->id}}">{{$pokemon->name}}</h5>
                                        <h5 class="modal-title mb-2 fs-6 text-muted" id="modalTitle-{{$pokemon->id}}">No. {{$pokemon->id}}</h5>
                                    ...
                                    <div class="modal-footer d-flex justify-content-center align-items-center gap-2">
                                        <form action="{{route('admin.pokedex.destroy', $pokemon->id)}}" method="post">
                                            @csrf
                                            @method('DELETE')
